...
func TestParseTable(t *testing.T) {
	cases := []struct {
		...
		{"about", []string{"-a", "7"}, Command{Action: ActionAbout, Ref: "7"}},
		{"about after the ref", []string{"7", "-a"}, Command{Action: ActionAbout, Ref: "7"}},
		{"about long", []string{"--about", "exam42"}, Command{Action: ActionAbout, Ref: "exam42"}},
		...
		{"move", []string{"--move", "7", "2"}, Command{Action: ActionMove, Ref: "7", To: "2"}},
		{"move by name", []string{"--move", "exam42", "2"},
			Command{Action: ActionMove, Ref: "exam42", To: "2"}},
		{"move help", []string{"--move", "-h"},
			Command{Action: ActionHelp, HelpFor: ActionMove}},
...
func TestParseErrors(t *testing.T) {
	cases := []struct {
		...
		{"about without a ref", []string{"-a"}},
		{"about and cd together", []string{"-c", "7", "-a"}},
		...
		{"move needs a target", []string{"--move", "7"}},
		{"move takes exactly two", []string{"--move", "7", "2", "3"}},
